...
const mappedData = Object.values(localData.members).map((member) => {
...
function createScoreboardMarkdown(mappedData) {
    return mappedData.map((it, index) => `${index + 1}) ${it.score.toString().padEnd(3, " ")}\t${createStars(it.days)} \t\t${it.name}`).join("\n")
}
